...
export const StatisticalInsights: React.FC<StatisticalInsightsProps> = ({
  ...
  const getRecommendationIcon = (level: string) => {
    switch (level) {
      ...
      case 'low': return <AlertCircle className="w-5 w-5 text-red-600" />;
      default: return <Target className="w-5 h-5 text-blue-600" />;
    }
  };

  const getConditionDisplayName = (condition: string, raceConditions: any): string => {
  ...
  return (
    <div className="space-y-4">
      ...
      <ResponsiveCard>
        ...
        <div className="space-y-3">
          <FlexLayout direction="row" justify="between" align="center">
            ...
            <span className={`px-2 py-1 rounded-full text-xs font-medium ${getConfidenceColor(confidenceLevel)}`}>
      ...
      {/* 的中パターン分析 */}
      <ResponsiveCard>
        <h3 className="text-responsive-lg font-bold mb-4">的中パターン実績</h3>
      ...
      {Object.keys(currentStats.conditionSpecificStats).length > 0 && (
        <ResponsiveCard>
          <h3 className="text-responsive-lg font-bold mb-4">今回のレース条件での実績</h3>
          
          <div className="space-y-2">
            {Object.entries(currentStats.conditionSpecificStats).map(([condition, rate]) => {
              const conditionName = getConditionDisplayName(condition, raceConditions);
              const isGoodCondition = rate > currentStats.overallHitRate;
              
              return (
                <FlexLayout key={condition} direction="row" justify="between" align="center">
      ...
      {/* 投資推奨 */}
      <ResponsiveCard className="bg-blue-50">
        <div className="p-4">
